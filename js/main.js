...
document.addEventListener('mousemove', e => {
    mouseX = cw * e.clientX / parseFloat(getComputedStyle(canvas).width);
    mouseY = ch * e.clientY / parseFloat(getComputedStyle(canvas).height);
});
...
function play() {
    ...
    if (state === 0) {
        ...
        if (dvd.x < 0)
            dvd.x = 0;
        if (dvd.x + dvd.width > 1000)
            dvd.x = 1000 - dvd.width;
        if (dvd.y < 0)
            dvd.y = 0;
        if (dvd.y + dvd.height > 1000)
            dvd.y = 1000 - dvd.height;
    }
}
